...
/*
Given ( 준비 단계 )
: 테스트를 수행하기 위해 필요한 사전 조건이나 초기 상태를 설정
: 주로 테스트 대상 객체를 생성하고나, 초기 데이터를 설정

When ( 실행 단계 )
: 실제로 테스트하려는 행동 또는 동작 수행
: 주로 테스트 대상 메서드를 호출하거나, 특정 이벤트를 발생시킴

Then ( 검증 단계 )
: 테스트 결과를 검증하는 단계
: 기대한 결과가 실제로 발생했는지 확인하기 위해 다양한 검증 수행
...
class BlogApiControllerTest {
    // MockMvc : Spring MVC 테스트에서 사용하는 강력한 도구, 웹 애플리케이션의 MVC 레이어를 테스트하기 위한 목적으로 사용
    //           주로 컨트롤러 단위 테스트, 통합 테스트에 사용
    //           실제 서버를 시작하지 않고도 서블릿 환경을 시뮬레이션하여 요청 처리(Get, Post, Put, Delete)
    @Autowired
    protected MockMvc mockMvc;

    @Autowired
    protected ObjectMapper objectMapper; // 직력화, 역직렬화를 위한 클래스
    // 직렬화 : 자바 객체를 JSON 데이터로 변환, 역직렬화 : JSON 데이터를 자바에서 사용하기 위해 자바 객체로 변환
    ...
    public void addArticle() throws Exception {
        ...
        // 객체 JSON으로 직렬화 ( writeValueAsString() )
